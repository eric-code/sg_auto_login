...
# 使用方法
在exe文件同目录使用config_template.json的格式新建一个名为config.json的文件，双击exe文件即可执行,可用windows任务计划程序进行定时调度

# todo
1. 把省平台浏览器中请求客户机本机wss://127.0.0.1:21061/xtxapp/的请求通过python程序直接转发到插着ukey并安装过驱动的机器上
2. 插着ukey并安装过驱动的机器上通过windows自带的netsh进行端口转发（驱动内部apache配置了只监听127.0.0.1）


## 如何在机器 B （插着 UKey 的那台）上正确配置（保姆级教程）
